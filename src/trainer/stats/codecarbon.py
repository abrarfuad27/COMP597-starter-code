...
class SimpleFileOutput(BaseOutput): 
    ...
    def __init__(self, 
        ...
        self.extra_metrics_queue = [] # Internal storage for hardware snapshots
        
        logger.info(
        f"Emissions data (if any) will be saved to file {os.path.abspath(self.save_file_path)}"
        )

    def add_extra_metrics(self, metrics: dict):
        self.extra_metrics_queue.append(metrics)
    ...
    def task_out(self, data: List[TaskEmissionsData], experiment_name: str):
        ...
        # Merge CodeCarbon data with our Hardware queue
        records = []
        for i, data_point in enumerate(data):
            row = dict(data_point.values)
            if i < len(self.extra_metrics_queue):
                row.update(self.extra_metrics_queue[i])
            records.append(row)
        ...
        if os.path.exists(save_task_file_path):
            existing_df = pd.read_csv(save_task_file_path)
            new_df = pd.concat([existing_df, new_df], ignore_index=True)
            
        new_df.to_csv(save_task_file_path, index=False)
        self.extra_metrics_queue.clear() # Clear after writing
...
class CodeCarbonStats(base.TrainerStats):
    ...
    def __init__(self, device : torch.device, run_num : int, project_name : str, output_dir : str) -> None: 
        ...
        self.step_handler = SimpleFileOutput(f"{run_number}cc_step_rank_{gpu_id}.csv", output_dir)
        self.training_step_tracker = OfflineEmissionsTracker(
            ...
            region = "quebec", 
            save_to_file = False, 
            output_handlers=[self.step_handler],
            allow_multiple_runs = True, 
            api_call_interval = -1, 
            gpu_ids = [gpu_id],
            log_level = "warning",
        ...
        self.substep_handler = SimpleFileOutput(f"{run_number}cc_substep_rank_{gpu_id}.csv", output_dir)
        self.training_substep_tracker = OfflineEmissionsTracker(
            ...
            region = "quebec", 
            save_to_file = False, 
            output_handlers=[self.substep_handler],
            allow_multiple_runs = True, 
            api_call_interval = -1, 
            gpu_ids = [gpu_id],
            log_level = "warning",
        ...
        self.log_frequency = 2  # <--- Define frequency here
        self.is_tracking_this_step = False


    def _get_hw_snapshot(self):
        torch.cuda.synchronize(self.device)
        
        # 1. GPU METRICS
        # Instantaneous kernel activity %
        gpu_util = pynvml.nvmlDeviceGetUtilizationRates(self.gpu_handle).gpu
        
        # Track both current tensors and the total cache reserved by PyTorch
        gpu_active_mb = torch.cuda.memory_allocated(self.device) / (1024**2)
        gpu_reserved_mb = torch.cuda.memory_reserved(self.device) / (1024**2)

        # 2. CPU & PROCESS TREE METRICS
        processes = [self.process] + list(self.process.children(recursive=True))
        total_cpu_pct = 0
        total_process_ram_mb = 0
        total_read_mb = 0
        
        for p in processes:
            try:
                with p.oneshot():
                    # Note: interval=None measures change since last call
                    total_cpu_pct += p.cpu_percent(interval=None)
                    total_process_ram_mb += p.memory_info().rss / (1024**2)
                    total_read_mb += p.io_counters().read_bytes / (1024**2)
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                continue

        # 3. SYSTEM-WIDE RAM (GB)
        system_ram_gb = psutil.virtual_memory().used / (1024**3)

        return {
            "gpu_util_pct": gpu_util,
            "gpu_active_mem_mb": gpu_active_mb,     # Variation in tensor sizes
            "gpu_reserved_mem_mb": gpu_reserved_mb, # The actual "footprint" on the card
            "cpu_util_pct":psutil.cpu_percent(interval=None),          # Aggregate across all workers
            "process_ram_res_mb": total_process_ram_mb, 
    ...
    def start_step(self) -> None:
        self.iteration += 1
        # Check if we should log this specific step
        #self.is_tracking_this_step = (self.iteration % self.log_frequency == 0)
        
        # if self.is_tracking_this_step:
        #     torch.cuda.synchronize(self.device)
    ...
    def stop_step(self) -> None:
        # if self.is_tracking_this_step:
        #     torch.cuda.synchronize(self.device)
        #     # Only snap hardware metrics if we are actually logging the step
        #     self.step_handler.add_extra_metrics(self._get_hw_snapshot())
    ...
    def stop_forward(self) -> None: 
        torch.cuda.synchronize(self.device)
        #self.substep_handler.add_extra_metrics(self._get_hw_snapshot())
